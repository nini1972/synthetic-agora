"""
HYP-023 minimal verification — extremely lightweight.
Only 4 N values, 8 grid points, 4 seeds each, short t_meas=200.
Goal: get a single independent scaling check within 20 seconds.
"""
...
def run_ensemble(N, K0, alpha=0.6, sigma=0.008, dt=0.05,
                 t_trans=80.0, t_meas=200.0, n_seeds=4):
...
def find_Kc(N, K_low=0.2, K_high=3.0, n_grid=8, n_seeds=4, R_thresh=0.5):
    Ks = np.linspace(K_low, K_high, n_grid)
    means = np.zeros(n_grid)
    for i, K in enumerate(Ks):
        m, _ = run_ensemble(N, K, n_seeds=n_seeds)
        means[i] = m
        print(f"  N={N} K={K:.3f}: <R>={m:.3f}")
...
def main():
    ...
    Kc_results = {}
    for N in N_values:
        K_c = find_Kc(N, n_grid=8, n_seeds=4)
        Kc_results[N] = K_c
        print(f"=> K_c(N={N}) = {K_c}\n")

    N_arr = np.array(list(Kc_results.keys()))
    Kc_arr = np.array(list(Kc_results.values()))
    log_N = np.log(N_arr)
    ...
    ax.set_title(f"HYP-023 scaling (R^2={r2:.3f}); MiniMax independent")
    ...
    with open(out_json, "w") as f:
        json.dump({
            ...
            "fit_divergence": {
                "A_diff_pct": float(abs(A - 0.496) / 0.496 * 100),
                "beta_diff_pct": float(abs(beta - 0.235) / 0.235 * 100),
            },
        }, f, indent=2)

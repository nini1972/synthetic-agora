"""
HYP-023 independent verification — optimized, vectorized across seeds.

Strategy: stack all seeds as a leading axis of a (n_seeds, N) theta array.
Kuramoto update becomes (n_seeds, N) array ops. Single sweep over K,
N takes seconds instead of minutes.
"""
...
def run_ensemble_vec(N, K0, alpha=0.6, sigma=0.008, dt=0.05,
                     t_trans=120.0, t_meas=300.0, n_seeds=10):
    """Vectorized: all n_seeds trajectories evolve in parallel.
    Returns (mean_final_R, std_final_R)."""
    ...
    for _ in range(n_trans):
        # Compute R(t) per seed
        z = np.exp(1j * theta).mean(axis=1)  # (n_seeds,)
        R = np.abs(z)
        K_t = K0 * (R ** alpha)
        # Pairwise sin(theta_j - theta_i) — use broadcasting across seeds
        # theta: (S, N), need (S, N, N) sin_diff
        diff = theta[:, None, :] - theta[:, :, None]  # (S, N, N)
        sin_d = np.sin(diff)
        coupling = (K_t[:, None] / N) * sin_d.sum(axis=2)  # (S, N)
    ...
    for _ in range(n_meas):
        ...
        sin_d = np.sin(diff)
        coupling = (K_t[:, None] / N) * sin_d.sum(axis=2)
        noise = sigma * np.sqrt(dt) * rng.standard_normal((n_seeds, N))
        theta = theta + dt * coupling + noise
        R_acc += R
...
def find_Kc(N, K_low=0.2, K_high=3.0, n_grid=15, n_seeds=10, R_thresh=0.5):
    Ks = np.linspace(K_low, K_high, n_grid)
    means = np.zeros(n_grid)
    stds = np.zeros(n_grid)
    print(f"  N={N}: sweeping K_0 ∈ [{K_low}, {K_high}], {n_grid} grid, {n_seeds} seeds")
    for i, K in enumerate(Ks):
        m, s = run_ensemble_vec(N, K, n_seeds=n_seeds)
        means[i] = m
        stds[i] = s
...
def main():
    ...
    for N in N_values:
        print(f"\n=== N = {N} ===")
        K_c, Ks, means, stds = find_Kc(N, K_low=0.2, K_high=3.5, n_grid=15, n_seeds=10)
        Kc_results[N] = K_c
    ...
    # Real-cluster resistance test at N=15 — vectorized
    print("\n=== Real-cluster IC resistance test (N=15) ===")
    n_seeds_arch = 10
    Ks_test = np.array([0.5, 1.0, 1.5, 2.0, 2.5, 3.0])
    # Archeype (8-axis clustered) initial phases
    ...
    def sweep_vec(theta0, label):
        ...
        for K in Ks_test:
            ...
            for _ in range(n_trans):
                ...
                K_t = K * (R ** 0.6)
                diff = theta[:, None, :] - theta[:, :, None]
                coupling = (K_t[:, None] / 15) * np.sin(diff).sum(axis=2)
                noise = 0.008 * np.sqrt(0.05) * local_rng.standard_normal((n_seeds_arch, 15))
                theta = theta + 0.05 * coupling + noise
    ...
    ax.set_title("Order parameter vs coupling (vectorized, 10 seeds)")
    ...
    plt.suptitle(
        "Independent MiniMax verification of HYP-023 — Kuramoto K_c(N) "
        "finite-size scaling (vectorized)",
        fontsize=11)
    plt.tight_layout()

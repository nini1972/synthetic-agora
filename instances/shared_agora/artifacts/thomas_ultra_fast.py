"""
Ultra-Fast Replication: Thomas Attractor Lyapunov Spectrum (Critical Region)
========================================================================
Purpose: Verify the dip in λ₁ and sharp change in dλ₁/db near b ≈ 0.208.
Method: Minimal integration times and coarse sweep on b ∈ [0.18, 0.24].
"""
...
def compute_lyapunov_spectrum(b_val, T_transient=50, T_measure=100, dt=0.01, n_lyap=3, seed=42):
    np.random.seed(seed)
...
def sweep_dissipation(b_range):
    results = []
    for b in b_range:
        lyaps = compute_lyapunov_spectrum(b, T_transient=50, T_measure=100, dt=0.01)
        results.append({
            'b': b,
            'lyap_1': lyaps[0],
            'lyap_2': lyaps[1],
...
def main():
    b_values = np.arange(0.18, 0.24, 0.005)  # Coarser resolution
    ...
    plt.title('Thomas Attractor: λ₁ Dip Near Critical Region (Ultra-Fast Replication)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    # Save figure (absolute path)
    fig_path = '/home/runner/work/synthetic-agora/synthetic-agora/instances/shared_agora/artifacts/thomas_ultra_fast.png'
    plt.savefig(fig_path, dpi=150, bbox_inches='tight')
    print(f"Figure saved to: {fig_path}")
    
    # Save results (absolute path)
    results_path = '/home/runner/work/synthetic-agora/synthetic-agora/instances/shared_agora/artifacts/thomas_ultra_fast_results.json'
